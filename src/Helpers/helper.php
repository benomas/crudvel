...
/*
*The principal porpouse is to bind DFP html from array.
*/
if(!function_exists('requestToModel')){
	...
	function requestToModel($instances){
      ...
      if(\Illuminate\Support\Facades\Input::has($intance))
        $data[$field]=\Illuminate\Support\Facades\Input::get($intance);
		}
...
if(!function_exists('crudvelResource')){
    ...
    if(!count($conditionals)){
      ...
      if(validateGetActionResource("activate",$only,$excludes))
        Route::get("{$resource}/{{$rowName}}/activate", "{$controller}@activate");

      if(validateGetActionResource("create",$only,$excludes))
        Route::get("{$resource}/create", "{$controller}@create");

      if(validateGetActionResource("deactivate",$only,$excludes))
        Route::get("{$resource}/{{$rowName}}/deactivate", "{$controller}@deactivate");

      if(validateGetActionResource("edit",$only,$excludes))
        Route::get("{$resource}/{{$rowName}}/edit", "{$controller}@edit");

      if(validateGetActionResource("export",$only,$excludes))
        Route::get("{$resource}/export", "{$controller}@export");

      if(validateGetActionResource("import",$only,$excludes))
        Route::get("{$resource}/import", "{$controller}@import");

      if(validateGetActionResource("index",$only,$excludes))
        Route::get($resource, "{$controller}@index");

      if(validateGetActionResource("show",$only,$excludes))
        Route::get("{$resource}/{{$rowName}}/show", "{$controller}@show");

      if(validatePostActionResource("destroy",$only,$excludes))
        Route::delete("{$resource}/{{$rowName}}", "{$controller}@destroy");

      if(validatePostActionResource("exporting",$only,$excludes))
        Route::post("{$resource}/export", "{$controller}@exporting");

      if(validatePostActionResource("importing",$only,$excludes))
        Route::post("{$resource}/import", "{$controller}@importing");

      if(validatePostActionResource("store",$only,$excludes))
        Route::post($resource, "{$controller}@store");

      if(validatePostActionResource("update",$only,$excludes))
        Route::put($resource."/{".$rowName."}", "{$controller}@update");
    }
	}
}
...
if(!function_exists('apiCrudvelResource')){
    ...
    if(!count($conditionals)){
      ...
      Route::get("{$prefixRoute}/{{$rowName}}/export", "{$controller}@export")->name("{$resource}row.export");
      ...
      if(in_array("export",$conditionals))
        Route::get("{$resource}/{{$rowName}}/export", "{$controller}@export")->name("{$resource}.row.export");
